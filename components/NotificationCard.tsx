...
export function NotificationCard({
  ...
  return (
    <Card className={cn("w-full border-none py-0 shadow-none outline-none")}>
      ...
      <CardContent
        className={cn(
          "grid gap-1.5 border-t p-0 py-1.5 ",
          notifications.length > 4 &&
            "border-b border-slate-200 max-h-[220px] overflow-y-scroll"
        )}
      >
        <div>
          {notifications.map((notification, index) => (
            <div key={index}>
              ...
              >
                <span className="flex h-2 w-2 translate-y-1 rounded-full bg-[#999999]" />
      ...
      <CardFooter className="px-0 pb-1">
        <Button
          ...
          className={cn(
            "mx-0 w-full text-white active:bg-custom-hover-green md:hover:bg-custom-hover-green ",
            notifications.length > 4 && "mt-3"
          )}
        >
